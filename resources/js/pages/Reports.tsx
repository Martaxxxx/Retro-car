...
const ReportsPage: React.FC = () => {
    ...
    const staticProjects = Object.values(projectMap);
    const allProjects = [
        ...staticProjects,
    ...
    const getProgressSummary = () => {
        ...
        const projectsProgress = selectedProjects.map(project => {
            // domyślnie parts są na project.parts lub project.partsList lub podobnie – dopasuj!
            const parts = (project as any).parts || [];
            // Jeśli masz daty dla parts, możesz je tu filtrować po createdAt, jak w shoppingList
    ...
    const handleGenerateProgressPdf = () => {
        ...
        doc.setFontSize(12);
        doc.text(`Zakres dat: ${startDate?.toLocaleDateString() || "-"} – ${endDate?.toLocaleDateString() || "-"}`, marginLeft, 30);
    ...
    const getSummaryData = () => {
        ...
        selectedProjects.forEach(project => {
            const items = (project as any).shoppingList?.filter((item: ShoppingItem) => {
    ...
    const handleGeneratePdf = () => {
        ...
        autoTable(doc, {
            startY: 40,
            head: [["Projekt", "Netto", "Brutto", "Pozycji", "Do zamówienia", "Zamówione", "Dostarczone"]],
            body: data,
            styles: {
                font: "Roboto",
                fontSize: 10,
            ...
            <div className="container mt-5 pt-5">
                ...
                >
                    ...
                    <div className="d-flex flex-column mt-3 mb-4 gap-3">
                        <div>
                            <input type="radio" id="cost" name="reportType" value="costs" onChange={() => setReportType("costs")} />
                            <label htmlFor="cost" className="ms-2">Raport kosztów</label>
                        </div>
                        <div>
                            <input type="radio" id="progress" name="reportType" value="progress" onChange={() => setReportType("progress")} />
                            <label htmlFor="progress" className="ms-2">Raport postępów</label>
                        </div>
                    </div>
                        ...
                        <>
                            ...
                            <div className="d-flex flex-column gap-2 rounded border bg-white p-3">
                                <div>
                                    ...
                                    <label htmlFor="selectAll" className="fw-semibold ms-2">Zaznacz wszystkie</label>
                                </div>
                                {allProjects.map(project => (
                                    <div key={project.id}>
                                        ...
                                        />
                                        <label htmlFor={project.id} className="ms-3">{project.name}</label>
                                    </div>
                                ))}
                            </div>
                        </>
                    ...
                    {reportType === "costs" && selectedProjectIds.length > 0 && (() => {
                        ...
                        return (
                            <div className="bg-light mt-5 rounded border p-4">
                                ...
                                <p><strong>Zakres dat:</strong> {startDate?.toLocaleDateString() || "—"} – {endDate?.toLocaleDateString() || "—"}</p>
                                <p><strong>Łączna kwota netto:</strong> {summary.totalNetAll.toFixed(2)} zł</p>
                                <p><strong>Łączna kwota brutto:</strong> {summary.totalGrossAll.toFixed(2)} zł</p>
                                <p><strong>Liczba pozycji:</strong> {summary.totalCountAll}</p>
                    ...
                    {reportType === "progress" && selectedProjectIds.length > 0 && (() => {
                        ...
                        return (
                            <div className="bg-light mt-5 rounded border p-4">
                                ...
                                <p><strong>Zakres dat:</strong> {startDate?.toLocaleDateString() || "—"} – {endDate?.toLocaleDateString() || "—"}</p>
                                <p><strong>Średni postęp wybranych projektów:</strong> {summary.globalProgress}%</p>
                                <p><strong>Łączna liczba części:</strong> {summary.totalParts}</p>
                                <p><strong>Statusy części:</strong></p>
                                <ul>
