...
const ProjectDetails: React.FC = () => {
  ...
  const fetchProjectAndUsers = async () => {
    try {
      ...
      const projectRes = await axios.get(
        `/api/projectdetails/${projectId}/${encodeURIComponent(name || "")}`
      );
  ...
  // Save project changes and sync global state
  const saveProjectChanges = async () => {
    ...
    try {
      ...
      alert("Błąd przy zapisie projektu! Sprawdź backend.");
  ...
  const removePart = async (id: string) => {
    try {
      ...
      setProject((prev) =>
        prev
          ? { ...prev, parts: prev.parts.filter((p) => p.id !== id) }
          : null
      );
  ...
  const handleRemoveFile = async (index: number) => {
    setFiles((prev) => {
      ...
      if (fileToRemove.id) {
        axios
          .delete(`/projects/files/${fileToRemove.id}`)
          .catch((err) =>
            console.error("Błąd usuwania pliku na serwerze:", err)
          );
      }
  ...
  const savePartsEdits = async () => {
    ...
    try {
      const newParts = project.parts.filter(
        (p) => typeof p.id === "string" && p.id.startsWith("temp-")
      );
      const existingParts = project.parts.filter(
        (p) => !(typeof p.id === "string" && p.id.startsWith("temp-"))
      );
      ...
      setProject((prev) =>
        prev
          ? {
              ...prev,
              parts: [...existingParts, ...createdParts],
            }
          : null
      );
  ...
  const handleDeleteProject = async () => {
    ...
    if (
      !window.confirm(
        "Czy na pewno chcesz usunąć ten projekt? Operacja nieodwracalna!"
      )
    )
      return;
    try {
    ...
    <>
      ...
      <div className="container mt-5 pt-5">
        <div className="row">
          ...
          >
            {editProjectMode ? (
              <>
                ...
                <div className="mb-3" style={{ maxWidth: "500px" }}>
                  ...
                  <Select
                    ...
                    onChange={(selected) => {
                      setUserIds(selected.map((s: any) => s.value));
                    }}
              ...
              <>
                ...
                <p>
                  <strong>Marka:</strong> {project.brand}
                </p>
                <p>
                  <strong>Model:</strong> {project.model}
                </p>
                <p>
                  <strong>Rocznik:</strong> {project.year}
                </p>
                <p>
                  <strong>Zlecenie:</strong> {project.carId}
                </p>
                <p>
          ...
          <div
            className="calendar-wrapper order-md-3 order-2 d-flex align-items-start"
        ...
        <div
          className="time-left mt-4"
          style={{ color: "#b03a2e", fontWeight: "bold" }}
        >
